...
def calculate_added_opinion_share_rate_excel(
    ...
    if AddedOpinionShareRateTool.ACCOUNT_URL_COLUMN not in account_df.columns:
        raise ValueError(f"账号 Excel 缺少必要列：{AddedOpinionShareRateTool.ACCOUNT_URL_COLUMN}")
    missing_posts = [col for col in AddedOpinionShareRateTool.REQUIRED_POST_COLUMNS if col not in post_df.columns]
    if missing_posts:
        raise ValueError(f"贴文 Excel 缺少必要列：{', '.join(missing_posts)}")
    ...
    work = post_df.copy()
    work["__homepage_key__"] = work[AddedOpinionShareRateTool.POST_URL_COLUMN].map(_normalized_key)
    work["__creation_type__"] = work[AddedOpinionShareRateTool.CREATION_TYPE_COLUMN].map(
    ...
    share_work["__has_added_opinion__"] = share_work.apply(
        lambda row: _is_non_empty_cell(row.get(AddedOpinionShareRateTool.TITLE_COLUMN))
        or _is_non_empty_cell(row.get(AddedOpinionShareRateTool.BODY_COLUMN)),
        axis=1,
    )
    ...
    output_df = account_df.copy()
    if AddedOpinionShareRateTool.OUTPUT_COLUMN in output_df.columns:
        output_df = output_df.drop(columns=[AddedOpinionShareRateTool.OUTPUT_COLUMN])
    account_keys = output_df[AddedOpinionShareRateTool.ACCOUNT_URL_COLUMN].map(_normalized_key)
    output_df[AddedOpinionShareRateTool.OUTPUT_COLUMN] = account_keys.map(lambda key: ratios_by_homepage.get(key, ""))
    ...
    rated_accounts = int(output_df[AddedOpinionShareRateTool.OUTPUT_COLUMN].map(_is_non_empty_cell).sum())
    return {
        "accounts": len(account_df),
        "posts": len(post_df),
...
def calculate_source_media_camp_ratios_excel(
    ...
    if SourceMediaCampRatioTool.ACCOUNT_URL_COLUMN not in account_df.columns:
        raise ValueError(f"账号 Excel 缺少必要列：{SourceMediaCampRatioTool.ACCOUNT_URL_COLUMN}")
    missing_posts = [col for col in SourceMediaCampRatioTool.REQUIRED_POST_COLUMNS if col not in post_df.columns]
    if missing_posts:
        raise ValueError(f"贴文 Excel 缺少必要列：{', '.join(missing_posts)}")
    missing_dictionary = [col for col in SourceMediaCampRatioTool.REQUIRED_DICT_COLUMNS if col not in dictionary_df.columns]
    ...
    dictionary_work["__shared_account_key__"] = dictionary_work[SourceMediaCampRatioTool.SHARED_ACCOUNT_COLUMN].map(
        _normalized_key
    )
    ...
    post_work["__homepage_key__"] = post_work[SourceMediaCampRatioTool.POST_URL_COLUMN].map(_normalized_key)
    post_work["__shared_account_key__"] = post_work[SourceMediaCampRatioTool.SHARED_ACCOUNT_COLUMN].map(
        _normalized_key
    )
    post_work["__creation_type__"] = post_work[SourceMediaCampRatioTool.CREATION_TYPE_COLUMN].map(
        lambda value: str(value).strip().lower() if _is_non_empty_cell(value) else ""
    )
    ...
    if drop_columns:
        output_df = output_df.drop(columns=drop_columns)
    account_keys = output_df[SourceMediaCampRatioTool.ACCOUNT_URL_COLUMN].map(_normalized_key)
